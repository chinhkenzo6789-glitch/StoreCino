...
const API_BASE_URL = "https://script.google.com/macros/s/AKfycbzMjBqpKKM7xJCkhx9xfU2A0Kkf-wMRkyYb2i5L9_WcXIlZtYB3EHibW4jX_yM-1em2Pg/exec";
...
const catalogProgressListeners = new Map();
const statsRequests = new Map();
const trackingRequests = [];
const loadedViewKeys = new Set();
const failedViewKeys = new Set();
...
async function requestApiData(url) {
  if (typeof fetch === "function") {
    try {
      ...
      // Fallback sang JSONP cho các deployment không trả CORS phù hợp.
    }
  }
...
function getCurrentViews(type, item) {
  ...
  if (!productViews.has(key)) {
    setCurrentViews(type, item, Number(item.views) || 0);
  }
...
function markViewsLoading(type, item) {
  const key = getProductKey(type, item);
  loadedViewKeys.delete(key);
  failedViewKeys.delete(key);
}
...
function markViewsFailed(type, item) {
  const key = getProductKey(type, item);

  if (!loadedViewKeys.has(key)) {
    failedViewKeys.add(key);
  }
}

function hasFailedViews(type, item) {
  return failedViewKeys.has(getProductKey(type, item));
}
...
function getViewsDisplay(type, item) {
  if (hasLoadedViews(type, item)) {
    return formatViews(getCurrentViews(type, item));
  }

  if (hasFailedViews(type, item)) {
...
function sendTrackingRequest(url) {
  const appendTrackingScript = () => {
    const callbackName = `__cinoTrack_${Date.now()}_${trackingRequests.length + 1}`;
    const script = document.createElement("script");
    const scriptUrl = new URL(url);
    ...
    function cleanup() {
      ...
      const index = trackingRequests.indexOf(script);
      if (index >= 0) {
...
function finalizeCatalogViews(type, catalog, statsItems = []) {
  ...
  catalog.forEach((item) => {
    const key = getProductKey(type, item);
    const pendingCount = pendingViewTracks.get(key) || 0;
    const localViews = getCurrentViews(type, item) + pendingCount;
    const nextViews = statsMap.has(key)
      ? Math.max(statsMap.get(key), localViews)
      : localViews;
...
function addCatalogProgressListener(type, listener) {
  if (typeof listener !== "function") {
    return () => {};
  }

  const listeners = catalogProgressListeners.get(type) || new Set();
  listeners.add(listener);
  catalogProgressListeners.set(type, listeners);

  return () => {
    const currentListeners = catalogProgressListeners.get(type);
    ...
    if (currentListeners.size === 0) {
      catalogProgressListeners.delete(type);
    }
  };
}

function emitCatalogProgress(type, catalog) {
  const listeners = catalogProgressListeners.get(type);
  ...
  listeners.forEach((listener) => {
    try {
      ...
      console.error(`Loi progress listener ${type}:`, error);
    }
  });
}

function preloadCachedCatalogs() {
  Object.keys(DATA_MAP).forEach((type) => {
    if (productCatalogs.get(type)?.length) {
      return;
    }
...
function renderProductSkeletons(type, count = 6) {
  ...
  const skeletons = Array.from({ length: count }, () => `
    <article class="product is-skeleton" aria-hidden="true">
      <div class="product-media">
        <div class="skeleton-box skeleton-media"></div>
      </div>
      <div class="product-info">
        <div class="product-meta product-meta-stack">
          <span class="product-views skeleton-line skeleton-line-short"></span>
        </div>
...
function requestCatalog(type, options = {}) {
  ...
  const detachProgressListener = addCatalogProgressListener(type, onProgress);
  ...
  if (catalogRequests.has(type)) {
    return catalogRequests.get(type).finally(detachProgressListener);
  }

  const request = fetchCatalogFromR2(type, {
    onProgress: (partialCatalog) => {
      primeCatalogState(type, partialCatalog);
      emitCatalogProgress(type, partialCatalog);
    }
  })
    .then((catalog) => {
  ...
  return request.finally(detachProgressListener);
}
...
async function loadStatsInBackground(type) {
  ...
  const url = buildApiUrl("views", {
    category: DATA_MAP[type],
    ts: Date.now()
  });
  const request = (async () => {
    try {
      const data = await requestApiData(url);
      if (data && typeof data === "object" && !Array.isArray(data) && data.error) {
        throw new Error(String(data.error));
      }
